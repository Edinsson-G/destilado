...
if parser.parse_args().carpetaAnterior==None:#si se va iniciar un destilado nuevo
    ...
    for nombre_variable in para_guardar:
        if nombre_variable not in  os.listdir(ruta):
            ...
            print("se creó",nombre_variable)
    if "guardar_historial" not in os.listdir(ruta):
        torch.save(parser.parse_args().historial,ruta+"guardar_historial")
        print("se guardó guardar_historial")
    if "imgs" not in os.listdir(ruta):
    ...
    #######################
    perdida=torch.load(ruta+"perdida",map_location="cpu")
    print(len(perdida))
    if "accuracyEntrenamiento"not in os.listdir(ruta):
        torch.save([],ruta+"accuracyEntrenamiento")
        print("guardado accuracyEntrenamiento")
    if "accuracyTesteo" not in os.listdir(ruta):
        torch.save([],ruta+"accuracyTesteo")
        print("guardado accuracy testeo")
    if "ol" not in os.listdir(ruta):
        torch.save(ol_inic,ruta+"ol")
